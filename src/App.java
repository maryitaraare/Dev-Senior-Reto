...
public class App {
        ...
        do {
            ...
            switch (option) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    System.out.println("¡RECOMENDACIÓN! DEBERIAS ELEGIR LAS DEMÁS OPCIONES DE NUESTRO MENÚ PARA TENER UNA EXPERIENCIA INTERESPACIAL");
                    break;
                default:
                    System.out.println("¡¡¡DEBES ELEGIR UNA DE LAS OPCIONES DE NUESTRO MENÚ!!!");
                    break;
            }
    ...
    public static void chooseShip() {
        ...
        for (int i = 0; i < spaceships.length; i++) {
            System.out.println("NAVE: " + (i + 1) + ". " + spaceships[i] + " - VELOCIDAD: " + shipSpeeds[i] + "km/h"
                    + " - CAPACIDAD DE PASAJEROS: " + passengers[i]);
        }

        try {
            System.out.println("SELECCIONA UNA NAVE (número): ");
            int select = scanner.nextInt();
            
            if(select < 1 || select > spaceships.length){
                System.out.println("HAS SELECCIONADO UNA OPCIÓN INVÁLIDA, POR FAVOR INTÉNTALO DE NUEVO.");
                return; 
            }


            System.out.println("INGRESA LA CANTIDAD DE PASAJEROS");
            int passenger_quantity = scanner.nextInt();

            if(passenger_quantity  > passengers[select - 1] ){
                System.out.println("LO SENTIMOS, LA CAPACIDAD MAXIMA PARA VIAJAR EN "
                + spaceships[select - 1].toUpperCase() + " ES " + passengers[select - 1] + " PASAJEROS.");
               } else {

                System.out.println("HAS SELECCIONADO LA NAVE " 
                + spaceships[select - 1].toUpperCase() + " CON " 
                + passenger_quantity + " PASAJEROS A BORDO.");
                  selectedShip = select - 1; // Guardar la nave seleccionada
            }
        } catch (Exception e) {
...
// Trabajo en Equipo con Santiago